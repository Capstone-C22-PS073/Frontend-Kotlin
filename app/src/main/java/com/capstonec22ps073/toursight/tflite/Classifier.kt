package com.capstonec22ps073.toursight.tflite
...
class Classifier(assetManager: AssetManager, modelPath: String, labelPath: String, inputSize: Int) {
    private var intrepreter: Interpreter
    ...
    init {
        ...
        intrepreter = Interpreter(loadModelFile(assetManager, modelPath), options)
    ...
    fun recognizeImage(bitmap: Bitmap): List<Recognition>{
        ...
        intrepreter.run(byteBuffer, result)
    ...
    private fun getSortedResult(labelProbArray: Array<FloatArray>):List<Classifier.Recognition>{
            ...
            if (confidence >= THRESHOLD) {
                pq.add(
                    Classifier.Recognition(
                        "" + i, if (labelList.size > i) labelList[i] else "Unknown", ((confidence).toFloat())
                    )
                )
            }
        }
        ...
        val recognition = ArrayList<Classifier.Recognition>()
